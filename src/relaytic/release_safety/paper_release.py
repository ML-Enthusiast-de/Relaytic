...
def _render_final_paper(
    *,
    ...
    reproduction_commands = _text_payload(inputs["paper_reproduction_commands"]).strip()
    ...
    return "\n".join(
        [
            ...
            f"{pay_pr}, and an Elliptic temporal graph-feature row reports test PR-AUC {ell_pr}; both are supporting evidence only. Elliptic2 subgraph evidence is retained as modern context only: the local repeated official-partition candidate reports PR-AUC {e2_pr} +/- {e2_std}, below the recorded RevClassifyDS reference of {ref_pr}, and reference-parity plus cohort gates remain unresolved. The contribution is therefore not a detector superiority claim. It is a claim-gated, reproducible AML evaluation environment that makes benchmark evidence, operational review metrics, limitations, and public wording inspectable together.",
            "",
            "## 1. Introduction",
            "",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "## Short Abstract",
            "",
            "The P13 Relaytic-AML draft presents a claim-gated evaluation environment, not a detector superiority claim. The current pack reports supporting PaySim synthetic temporal-fraud test PR-AUC "
