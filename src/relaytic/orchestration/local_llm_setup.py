...
DEFAULT_LLAMA_ALIAS = "relaytic-4b"
...
def _setup_remote_api(
    *,
    ...
    api_key = os.getenv("RELAYTIC_API_KEY", "").strip() or os.getenv("C2S_API_KEY", "").strip()
    ...
    if provider == "openai" and not api_key:
        report["errors"].append(
            "Missing API key. Set RELAYTIC_API_KEY (or OPENAI_API_KEY) to enable OpenAI calls."
        )
        report["suggested_env"] = {
            "RELAYTIC_PROVIDER": "openai",
            "RELAYTIC_MODEL": model,
            "RELAYTIC_ENDPOINT": endpoint,
            "RELAYTIC_API_KEY": "<set-your-key>",
        }
        return report

    if provider == "openai_compatible" and not api_key:
        report["warnings"].append(
            "No RELAYTIC_API_KEY provided. This is allowed for some endpoints, but many require bearer auth."
        )

    report["ready"] = True
    report["suggested_env"] = {
        "RELAYTIC_PROVIDER": provider,
        "RELAYTIC_MODEL": model,
        "RELAYTIC_ENDPOINT": endpoint,
    }
    if api_key:
        report["suggested_env"]["RELAYTIC_API_KEY"] = "<already-set>"
    return report
...
def _setup_ollama(
    *,
    ...
    if binary is None:
        ...
        report["suggested_env"] = {
            "RELAYTIC_PROVIDER": "llama_cpp",
            "RELAYTIC_MODEL": DEFAULT_LLAMA_ALIAS,
        }
        return report
