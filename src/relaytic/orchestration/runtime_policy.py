...
def apply_environment_overrides(
    ...
    provider = _env_get(values, "RELAYTIC_PROVIDER", "C2S_PROVIDER", policy.provider)
    default_profile = _env_get(values, "RELAYTIC_PROFILE", "C2S_PROFILE", policy.default_profile)
    offline_mode = _env_get(values, "RELAYTIC_OFFLINE_MODE", "C2S_OFFLINE_MODE")
    require_local_models = _env_get(
        values, "RELAYTIC_REQUIRE_LOCAL_MODELS", "C2S_REQUIRE_LOCAL_MODELS"
    )
    block_remote_endpoints = _env_get(
        values, "RELAYTIC_BLOCK_REMOTE_ENDPOINTS", "C2S_BLOCK_REMOTE_ENDPOINTS"
    )
    api_calls_allowed = _env_get(
        values, "RELAYTIC_API_CALLS_ALLOWED", "C2S_API_CALLS_ALLOWED"
    )
    telemetry_allowed = _env_get(
        values, "RELAYTIC_TELEMETRY_ALLOWED", "C2S_TELEMETRY_ALLOWED"
    )
    model_override = _env_get(values, "RELAYTIC_MODEL", "C2S_MODEL")
    remote_default_model = _env_get(
        values,
        "RELAYTIC_REMOTE_DEFAULT_MODEL",
        "C2S_REMOTE_DEFAULT_MODEL",
        policy.remote_default_model,
    )
    resolved_offline = (
        policy.offline_mode
