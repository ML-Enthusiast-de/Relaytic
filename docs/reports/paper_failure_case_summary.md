...
| Test-set selection violation | A model-selection path tries to use test evidence before the finalist is fixed. | Validation-only selection policy | Only validation evidence may select, calibrate, or threshold the finalist. | probe_surfaces=1 validation-only; test_used_for_selection=False; final_policy=only_validation_selected_finalist_is_evaluated_on_test | `pass` |
