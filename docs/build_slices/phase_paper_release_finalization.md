...
5. Require a clean public immutable commit for the exact out-of-tree paper build. A supplied tag is optional, but it must exist locally and remotely, resolve to that commit, and agree with the generated manuscript, PDF, source bundle, and release manifest.
...
3. Push that exact commit to the public remote.
4. Run:

```powershell
py -3.11 -m relaytic.ui.cli release-safety paper-release-integrity --final --format json
```
...
- A final build fails for a dirty worktree or a source commit absent from the public remote. When a tag is supplied, it also fails if the tag is absent or resolves elsewhere.
- The final release manifest contains PDF/source SHA-256 hashes and the full commit. It includes a tag only when that tag was verified locally and remotely.
