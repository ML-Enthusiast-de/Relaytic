...
On macOS or Linux, replace \nolinkurl{py -3.11} with \nolinkurl{python3}. The system-evaluation command writes task-level evidence, aggregate behavior evidence, rowless handoff evidence, partial-run recovery evidence, claim-gate case studies, and a fail-closed manifest. The README names the generated JSON files for readers who want to inspect them directly.
