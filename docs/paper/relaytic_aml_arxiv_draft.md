...
Financial-crime machine learning is often described through model scores, but real AML work is shaped by data access, temporal validity, graph context, review capacity, and the language people use when they explain a result. Relaytic-AML is a local-first evaluation lab for that setting. The system keeps the workspace as the source of truth, records data posture and modeling decisions as inspectable artifacts, and lets specialist agents help without taking authority away from the local record. Relaytic began as a general local inference-engineering system. In this work it is sharpened into an AML-focused environment where humans and external agents can see what is known, what is blocked, and what should happen next. The benchmark rows are evidence for the environment, not the whole story. The current public pack includes supporting PaySim synthetic temporal-fraud evidence with test PR-AUC 0.638773, supporting Elliptic temporal graph-feature evidence with test PR-AUC 0.668756, and Elliptic2 modern-context evidence at PR-AUC 0.94324 +/- 0.000882, below the recorded RevClassifyDS reference of 0.974. The main contribution is an auditable local architecture that keeps experiments, review assumptions, limitations, and public claims from drifting apart.
