...
- latest paper-track slice: P8, `amlsim_generation_manifest.json`, `amlsim_typology_manifest.json`, `elliptic2_subgraph_access_report.json`, `subgraph_benchmark_blocker_report.json`, and Paper Track P9 as the next target
...
- next planned academy follow-on: 16A, capability registry and capability cards, after Paper Track P9 through P13 complete operational proof, paper draft, dry run, and arXiv proof path
...
With Slice 15Z-R and Paper Track P0-P8 now landed, the next high-leverage frontier follow-ons are:

- Paper Track P9, add operational metrics without inflating P8's blocked hard-track posture into performance evidence
- Before final result tables, make an explicit Elliptic2 acquisition/pilot decision: it is the highest-upside recovery for modern subgraph breadth, while AMLSim can only add seeded synthetic typology and workflow evidence after its proxy audit passes
- Paper Track P10 through P13, preserve P7's graph-neural non-competitiveness finding unless recovered with stronger proof, generate reproducible tables, write a claim-linted draft, run clean-clone proof, and only then attempt an arXiv release
