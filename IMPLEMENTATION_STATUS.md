...
- completed paper-track slices: Paper Track P0 baseline freeze, Paper Track P1 public-surface cleanup, compatibility retention reporting, Relaytic API/tool aliases, Paper Track P2 thesis/claim contract freeze, Paper Track P3 dataset registry/access manifest freeze, Paper Track P4 PaySim-style temporal benchmark runner, Paper Track P5 Elliptic graph loader/provenance, Paper Track P6 strong tabular baseline suite, Paper Track P6-A competitive PaySim rerun/publishability gate, Paper Track P7 Elliptic graph baseline suite, Paper Track P8 AMLSim/Elliptic2 blocked-or-supported decisions, Paper Track P8-A Elliptic2 modern-benchmark recovery pilot, Paper Track P8-B Elliptic2 competitive and robustness suite, Paper Track P8-C modern subgraph reference-parity and leakage-resistant cohort gate, Paper Track P8-D paper thesis narrowing and alternative evidence decision, Paper Track P9 operational AML evaluation layer, Paper Track P10 reproducible paper table generator, Paper Track P11 paper draft and figure pack, Paper Track P12 external dry run and clean-clone proof, Paper Track P13 claim-safe paper release pack, Paper Track P14 final arXiv source bundle and release-candidate audit, Paper Track P15 measured system-evaluation proof pack, Paper Track P16 failure-case evaluation pack, Paper Track P17 governance machinery ablation pack, Paper Track P18 governance invariants and adjacent-systems positioning, Paper Track P19-A external score-file adapter proof pack, Paper Track P19-B hosted-score case-study integration, Paper Track P20 PaySim selection-story plus reader-guidance polish, Paper Track P21 final source/PDF preflight plus release changelog, Paper Track P22 author-review layout hardening plus regression closure, Paper Track P23 novelty plus adjacent-systems distinction hardening, Paper Track P24 release integrity plus revision-locked artifact generation, and Paper Track P25 evidence disclosure plus immutable tagged-release finalization
- flagship frontier direction: Relaytic-AML, the AML and financial-crime edition of Relaytic
- next recommended action: review and commit the P25 source candidate, create the reviewed release tag, run the exact-revision final build from that tag, then perform the arXiv upload. Slice 16A may resume after that release decision.
- latest guide-and-context slice: 15V-A, `relaytic guide`, graceful `relaytic status` fallback, artifact shortlist, starter questions, optional local-LLM guide summary, and redacted external LLM context pack
...
- latest AML pivot slice: 15N, AML domain contracts, case ontology, review-budget posture, and AML claim-scope visibility
- latest AML graph slice: 15O, deterministic entity graphs, typology detection, suspicious-subgraph scoring, and entity-case expansion
- latest AML casework slice: 15P, analyst-review queue policy, review-budget ranking, case packets, and review-capacity sensitivity
- latest AML stream-risk slice: 15Q, stream-risk posture, weak-label posture, delayed-outcome alignment, drift-triggered recalibration signals, and rolling alert-quality reporting
- latest AML proof-pack slice: 15R-A, PaySim-style and flattened Elliptic-style proof-pack alignment, cross-track public-claim gating, and proof visibility across benchmark, summary, assist, and mission control
...
- latest AML baseline-and-ablation slice: 15U, `relaytic aml baselines`, baseline matrices, capability ablations, optional-adapter fallback reporting, contribution summaries, and supported/proxy/blocked benchmark relevance scorecards
- latest AML graph-loader slice: 15V, `relaytic aml graph-loader`, raw graph bundle ingestion, flattened graph compatibility labeling, subgraph task manifests, graph provenance, claim scope, and graph benchmark cataloging
- latest AML temporal slice: 15W, `relaytic aml temporal`, delayed-label evaluation, positive-unlabeled posture, threshold-drift reporting, time-window scorecards, and temporal public-claim gating
- latest AML environment slice: 15X, `relaytic aml environment`, model-vs-environment score separation, workflow task matrices, benchmark-environment scoring, unsafe-steering rejection evidence, and failure reporting
- latest public-docs slice: 15Y, demo-first README path, Relaytic-AML thesis, product story, paper benchmark runbook, and handbook demo commands
...
- latest roadmap update: Paper Track P0 through P25 is implemented. P25 adds explicit prior-test-exposure disclosure, operating-point provenance, versioned-reference verification, executable rerun-status semantics, and a tagged exact-revision final-build command.
...
- latest paper-track slice: P25, evidence disclosure and immutable tagged-release finalization over the reviewed P24 source/PDF bundle
- latest paper-novelty-positioning slice: P23 adds a generated novelty-positioning audit, an adjacent-systems distinction matrix, P14/P21 preflight gates for those artifacts, and manuscript wording that frames Relaytic-AML as local AML evaluation-evidence governance around detectors and agent-assisted workflows, not as a detector replacement, SAR drafting assistant, generic MLOps tracker, model-card substitute, or general agent-governance product.
- latest paper-integrity slice: P24 corrected factual, protocol, bibliography, statistical-wording, figure/table, and manuscript-consistency defects without changing benchmark values. The tracked review candidate passes all 12 P24 report classes; final mode remains intentionally blocked until the reviewed changes are committed and the worktree is clean.
- latest paper-final-layout repair: compressed the main-body system evaluation into a synthesis table, moved dense audit details to appendix tables with real LaTeX captions, anchored figures to prevent prose-fragment floats, guarded platform command labels against page-bottom stranding, refreshed Figure 4 wording, made rowless handoff redaction explicit, regenerated the canonical Markdown/LaTeX/PDF bundle, and reran P21 final preflight plus rendered-page inspection of pages with figures, tables, appendix audit records, and command blocks.
- latest paper-final-citation/appendix polish: reduced remaining defensive detector-boundary phrasing, shortened the appendix reproduction commands while pointing full scripts to the README, refreshed source-verification dates to 2026-07-07, added access dates to living FATF/FinCEN/FFIEC references, preserved BibTeX title capitalization, regenerated the Markdown/arXiv/PDF bundle, visually inspected the shortened page-20 command/reference page, and reran P21 final preflight, focused paper regressions, font/metadata/log scans, and git-safety.
- latest paper-prose/source-integrity pass: smoothed abrupt introduction and system-overview transitions, removed catchphrase-like evidence wording, named AML metrics before interpreting them, added explicit PaySim/Elliptic result referents, added ambiguous-referent lint coverage for vague score/result phrases, added the missing MLflow citation for experiment-tracking comparisons, regenerated the Markdown/LaTeX/PDF bundle, verified that prose quote marks are absent outside code snippets, and reran P21 final PDF/source preflight.
- latest paper-release polish: enlarged Figure 4 labels and notes, preserved explicit separation between ranking and review-budget metrics, tightened the paper entry point into a cross-platform reproduction guide, regenerated the vector figure/PDF/source artifacts, visually inspected all 23 PDF pages, and reran P20, P23, P24, P14, P21, focused paper tests, smoke coverage, font/log scans, and git-safety checks.
- latest paper-architecture/editorial pass: removed first-person narration and reader-visible semicolons from the manuscript body, introduced every main table and figure in the surrounding argument, aligned manuscript table numbering with the compiled PDF, added implementation-grounded specialist-role, harness, strict-action, capability-scope, checkpoint, and bounded-loop detail, fixed the Figure 3 Elliptic2 label boundary, stabilized algorithm placement, regenerated the 24-page source/PDF candidate, and added deterministic editorial regression checks.
- latest paper-reader/release-snapshot pass: clarified that paper readers should start from the README, manuscript, and compact `docs/paper/README.md` artifact map; framed the visible paper artifact-generation code as reproducibility infrastructure rather than a first-read path; added fixed release-tag/archive guidance instead of recommending a separate cleaned repository; removed remaining reader-facing `paper generator` wording from the generated manuscript source; and extended P20 reader-guidance checks to guard the paper artifact map and stable snapshot policy.
- latest paper-PaySim slice: P6-A, full 6,362,620-row competitive rerun with prior-step destination history, 14 recorded probe trials, five full-training finalists, validation-selected Extra Trees (`validation_pr_auc=0.568725`, `test_pr_auc=0.638773`), a supporting-only paper-table candidate, and headline/hard claims still blocked
- latest paper-Elliptic slice: P7, full raw Elliptic graph suite over 203,769 nodes and 234,355 same-time-step edges; validation-selected LightGBM with source plus structural features reports `validation_pr_auc=0.976654`, `test_pr_auc=0.668756`, a descriptive `+0.004588` test lift over paired source-only features, and supporting-only posture while weaker GraphSAGE shadow and graph-neural/SOTA claims remain blocked
- latest paper-hard-graph decision: P8, neither Elliptic2 nor AMLSim is locally runnable evidence: Elliptic2 is blocked on acquisition, official-loader, overlap/split, resource-budget, and license proof; AMLSim is blocked until seeded generated outputs, hashes, typology distribution, generator commit, and generated-data license are frozen. Neither may appear as a performance row.
- latest paper-modern-graph recovery: P8-A superseded the Elliptic2 access portion of P8 by auditing the official labeled core (`121810` subgraphs, `2763` suspicious), pinning RevTrack/RevClassify as the modern reference, recording the original paper/code split discrepancy, verifying a CPU-bounded embedding-cache derivation, and completing an exploratory context pilot (`validation_pr_auc=0.952440`, `test_pr_auc=0.935255`); P8-B has now tested that pilot and retains only supporting evidence
- latest paper-modern-graph competitiveness: P8-B records reported full-shot `RevClassifyDS` PR-AUC `0.974`, documents that no classification checkpoint is distributed in the pinned repo and local hardware is CPU-only, discovers that the RevTrack-evaluable cohort covers `110902/121810` audited current-core rows, selects pooled-moments LightGBM on validation, and reports repeated official test `PR-AUC=0.943240 +/- 0.000882` plus row-order-independent content-hash test `PR-AUC=0.929669 +/- 0.000538`. It admits supporting modern-context evidence only; P8-C confirms that reference-parity and broad modern-subgraph claims remain blocked.
- latest paper-modern-reference parity: P8-C requested faithful RevClassify parity execution, records missing local preconditions (`lightning`, `hydra-core`, `omegaconf`, `torchmetrics`, CUDA-class accelerator, and distributed classification checkpoints), narrows claims to the pinned RevTrack-evaluable cohort because no official component-ID mapping proves equivalence to the current core, and finds that a strict entity-disjoint component split is degenerate (`110889/110902` rows in the largest component, `0.99988278`). P8-C blocked P9 until P8-D recorded an accepted thesis boundary.
- latest paper-thesis narrowing: P8-D accepts the narrowed first-paper thesis around claim-gated AML evaluation and operational evidence, consumes P8-B/P8-C gates directly, keeps Elliptic2 as supporting context and limitation evidence only, blocks Elliptic2 performance/SOTA/full-core/entity-disjoint claims, defers faithful RevClassify reprovisioning to a later extension, and unblocked P9.
- latest paper-operational slice: P9 materializes PaySim and Elliptic supporting review-budget rows, review-budget curves, false-positive burden proxies, case-packet completeness reporting, and `paper_operational_claim_guard.json`. It allows P10 table generation to proceed while hard business-value, headline operational, Elliptic2 performance-contribution, and SOTA claims remain blocked.
- latest paper-table slice: P10 generates reproducible supporting performance, operational, modern-context, and limitation tables from committed artifacts. Every numeric metric cell now carries dataset, split, command, run-directory, artifact, claim-state, budget-tier, leakage-posture, and publishability-gate provenance; `paper_metric_cell_audit.json` passes and P11 is unblocked while hard/headline claims remain blocked.
- latest paper-draft slice: P11 generates the first Relaytic-AML draft and deterministic figure pack from P10 evidence, materializes `paper_limitations_matrix.json`, and records a passing `paper_claim_lint_report.json`; P12 clean-clone proof is unblocked while hard AML, headline, SOTA, RevClassify parity, graph-neural superiority, and hard business-value claims remain blocked.
- latest paper-dry-run slice: P12 runs `relaytic release-safety paper-dry-run`, writes a clean-clone checklist, can execute an optional temp full-profile isolated install probe, regenerates the P10/P11 paper-smoke path, records leak-scan status, emits deterministic failure reporting, and unblocks P13 only for claim-safe evaluation-environment release language.
- latest paper-release slice: P13 runs `relaytic release-safety paper-release`, writes a claim-safe Markdown draft pack, citable references, generated paper tables, public attention pack, submission checklist, release manifest, and allowed-public-claims report; P14 supplies the deterministic arXiv source bundle and P15 supplies measured system-evaluation evidence, while final upload still requires human PDF review and a clean tag target. Hard AML, headline, SOTA, RevClassify parity, graph-neural superiority, production-ready, and hard business-value claims remain blocked.
- latest paper-arXiv-source slice: P14 runs `relaytic release-safety paper-arxiv-source`, writes a deterministic LaTeX source tree under `docs/paper/arxiv_src/`, converts the P11 SVG figure pack to PDF figures for the selected pdfLaTeX processor, audits citations and figure references, scans the generated source package for local paths, secrets, stale prototype wording, unguarded hard claims, unresolved evidence placeholders, compile-log warnings, and missing author/PDF metadata, and writes a release-candidate checklist that keeps upload blocked until final human PDF inspection and a clean tag target. A follow-up paper-excellence pass clarified the system-evaluation framing around evidence cells, claim gates, research questions, agentic ML reliability, and P14 source-candidate documentation without changing the blocked hard-claim contract.
- latest paper-excellence pass: expanded the generated arXiv draft with stronger current-frontier context, methodology, Relaytic architecture, special features, intended company/research uses, limitations, and future-work framing; redesigned the deterministic figure set and paper tables for review readability while preserving hidden metric-cell provenance; regenerated P13/P14 artifacts; and produced `docs/paper/relaytic_aml_arxiv_draft.pdf` with zero LaTeX overfull/underfull box warnings. Hard/headline AML claims remain blocked until later evidence gates pass.
- latest paper-architecture pass: reframed the final arXiv draft and public attention pack around Relaytic as a local-first agentic evaluation environment, added a specialist-role boundary table, moved the architecture and artifact graph thesis before benchmark context, redesigned Figure 1 around controlled local artifacts, redacted handoff, and fail-closed claim gates, and kept PaySim/Elliptic/Elliptic2 numbers as supporting evidence rather than the paper identity.
- latest paper-human-voice pass: rewrote the final arXiv draft around a more natural founder/researcher narrative, removed reader-facing paper-track labels and generator/checklist language, shortened reproducibility to repo paths and commands, added boxed table rendering in the PDF, cleaned figure captions and footers, updated README paper positioning around Relaytic as the general local-first system and Relaytic-AML as the current flagship edition, and preserved the blocked hard-claim contract.
- latest paper-AI-assistance policy pass: checked official arXiv moderation guidance, added a reader-facing AI-assistance disclosure to the generated arXiv draft, and added final-upload checklist reminders while keeping LLM tools out of authorship and preserving human responsibility for claims, artifacts, figures, tables, and code references.
- latest paper-professional-preflight pass: replaced unfinished evidence markers and metadata placeholders, moved the missing isolated destination-history ablation into limitations, shortened reader-facing evidence aliases while preserving machine metric-cell provenance, added explicit audit, blocked-claim, rowless-handoff, interrupted-recovery, and reproducibility tables, regenerated embedded-font vector PDF figures, and produced `docs/paper/relaytic_aml_arxiv_draft.pdf` from a clean LaTeX source build. The paper remains an evaluation-lab/governance/reproducibility systems paper, not a detector-superiority paper.
- latest paper-chief-scientist pass: checked current cs.LG and trending agent/ML paper posture, sharpened the public README so the repo tells the AML evaluation-lab story first, added a paper claim-contract and research-question matrix, connected Relaytic-AML to current agentic-evaluation style without borrowing unsupported hype, and kept all hard/headline AML claims blocked.
- latest paper-figure-readability pass: enlarged generated figure typography, simplified Figure 1 role labels so text stays inside boxes, increased figure note/value label sizes, rendered figures at full LaTeX line width, added normal-size bold figure captions, and regenerated the arXiv PDF/source artifacts so the visual story is readable in the draft.
- latest paper-human-readability pass: replaced the reader-facing questions table with a design-thesis section, reworded claim-governance language into evidence-bounded release claims, added PaperBench-backed agent-reliability context, tightened Section 8 table width rendering, removed the Section 14 future-work table in favor of prose, simplified Figure 1 labels to prevent box overflow, and rewrote the AI-assistance disclosure in a more professional voice.
- latest paper-source-accessibility pass: moved Figure 2/3 y-axis labels above the plot so they no longer cross the axis, rechecked all 15 bibliography URLs with HTTP status and browser-source inspection, replaced the CACM Datasheets URL with its accessible arXiv source while retaining DOI metadata, regenerated the canonical PDF artifact, and removed the temporary readability duplicate so the repo has one paper PDF.
- latest paper-technical-depth pass: expanded the canonical generated arXiv draft with the actual Relaytic agent runtime, deterministic/advisory loop model, planning handoff, modeling/search harness, trace/adjudication loop, guide/assist loop, and release harness; strengthened the methodology and operating-layer tables around failure modes and evidence controls; regenerated the P13/P14 artifacts; and rebuilt the single canonical PDF with clean LaTeX logs and visual inspection of the architecture/runtime pages.
- latest paper-system-depth pass: added a concrete execution-contract section with run lifecycle, agent surfaces, artifact contracts, reviewer questions, pseudocode, and a metric evidence-cell example; added a system-evaluation subsection covering guide/status recovery, redacted external-model help, metric auditability, trace inspectability, and claim blocking; added RE-Bench to the agentic research-engineering context; regenerated P13/P14 artifacts; and rebuilt the single canonical PDF after fixing a pseudocode line overflow.
- latest paper-agent/privacy polish pass: rewrote the generated abstract and main text so anti-money laundering, precision-recall area under the curve, large language model, Model Context Protocol, and machine learning acronyms are introduced naturally; reframed Relaytic-AML as cooperating specialist agents over a local artifact graph; expanded OpenClaw, Claude/Codex skill, JSON CLI, and MCP handoff/privacy guidance; added Windows PowerShell plus macOS/Linux reproduction commands; regenerated P13/P14 artifacts; and refreshed the canonical `docs/paper/relaytic_aml_arxiv_draft.pdf` after the preview lock cleared.
- latest paper-lab-style polish pass: compared the draft structure against current OpenAI, Anthropic, and Meta-style technical reports; replaced the algorithm table with concise pseudocode, removed question/interview-style capability and evaluation framing, toned down Figure 2/3 titles and typography, made README plus paper the only reader entry points in the reproducibility text, regenerated P11/P13/P14 artifacts, and rebuilt the canonical PDF with a clean LaTeX log and visual inspection of the affected pages.
- latest paper-AML-depth pass: expanded the generated arXiv draft with an AML problem-setting primer, transaction-pattern examples, entity/graph context, and decision-time leakage framing; made the systems contribution stronger around local-first specialist agents, artifact-first loops, evidence cells, external-agent handoff, and claim gates; added a second evidence-cell claim-gate algorithm; reduced repeated table-style presentation; unified generated figure typography; redesigned Figure 4 as a cleaner claim-boundary ladder; improved natbib citation rendering; regenerated P13/P14 artifacts; and compiled a clean fresh PDF at `docs/paper/arxiv_src/main.pdf` while `docs/paper/relaytic_aml_arxiv_draft.pdf` remained locked by the local PDF preview process.
- latest paper-result-interpretation pass: removed duplicate in-figure titles so captions carry figure names in the paper font, restored compact architecture/evidence tables where comparison helps, moved benchmark interpretation directly beside the evidence table and figures, clarified what each result means and does not mean, and preserved the blocked hard/headline claim contract.
- latest paper-tone/source-integrity pass: removed meta-comparative and self-referential wording from the generated arXiv draft, expanded the P13 reader-tone lint to 25 rules for recurring nonprofessional phrases, tightened source-summary prose to paraphrase rather than echo citation wording, regenerated P13/P14 artifacts, and rebuilt the canonical PDF with a clean final LaTeX log.
- latest paper-system-evaluation slice: P15 measures guide onboarding, partial-run recovery, rowless and redacted external-agent handoff, MCP-style server discovery, local-LLM advisory boundaries, reader/agent navigation tasks, metric-cell provenance recovery, and P11/P12/P13 claim-gate behavior; required deterministic protocol checks pass, raw rows and private paths stay out of committed system-evaluation reports, and P13 now gates the release pack on this measured system-behavior evidence.
- latest paper-reviewer navigation pass: rewrote the System Evaluation subsection so it explains the concrete failure modes, mechanisms, and recheck artifacts behind the P15 protocol instead of presenting a shallow pass table; expanded reproducibility with a focused P15 rerun/test path; and clarified in the README and paper that Relaytic is the broader local-first inference lab while Relaytic-AML is the flagship paper edition.
- latest paper-chief-scientist execution pass: added `paper_system_task_eval.json` as a deterministic reader/agent task suite, gated P13 on it, expanded the generated manuscript with an adjacent-systems comparison and a PaySim evidence-cell walkthrough, upgraded pseudocode rendering to framed LaTeX verbatim blocks, updated README navigation, and preserved all blocked hard/headline/SOTA claim boundaries.
- latest paper-arXiv systems rewrite pass: restructured the generated manuscript into a conventional arXiv systems/ML paper, tightened the abstract to the evidence-cell thesis, added explicit research questions and four contributions, materialized dataset/task, model/search, evidence-cell, PaySim-ablation, and system-evaluation tables from committed artifacts, redesigned the deterministic figure pack around architecture, evidence-cell schema, benchmark/review-budget evidence, and claim-gate examples, and recorded missing-evidence gates for author metadata, final upload, and future stronger claims before the later final-polish pass resolved the public placeholders.
- latest paper-final-polish pass: set the public author metadata to `Tobias Gehra` with contact `t.gehra.ai@gmail.com`, refreshed PDF metadata, widened and re-spaced the benchmark/review-budget figure, compacted Tables 1 and 3, added a short redacted evidence-cell snippet, tightened audit wording, regenerated the arXiv source/PDF with embedded vector figures, and preserved the local-first evaluation-lab claim boundary rather than detector-superiority wording.
- latest paper-publication-preflight pass: fixed the Figure 1 lower-callout overlap in the generated SVG/PDF, restored a reader-visible Use of AI Assistance section, rechecked all 19 bibliography sources, added the missing Model Cards source-verification record, refreshed source-verification access dates to 2026-06-26, rebuilt the canonical PDF/source bundle, and preserved all blocked hard/headline detector claims.
...
- next paper action: commit the reviewed P25 source, create the reviewed release tag, run `paper-release-integrity --final --release-tag <tag>`, inspect the exact-revision PDF, and perform the arXiv upload action
...
- paper-grade benchmark surfaces through `relaytic benchmark run` and `relaytic benchmark show`, with machine-stable benchmark manifests, ranked benchmark tables, ablation matrices, rerun variance reports, explicit claim-boundary reporting, and temporal benchmark posture that stays separate from deployment readiness
- deterministic AML graph surfaces through `entity_graph_profile.json`, `counterparty_network_report.json`, `typology_detection_report.json`, `subgraph_risk_report.json`, and `entity_case_expansion.json`, with structural-baseline versus graph-shadow comparisons that stay audit-ready instead of pretending a heavy graph model is already production-trusted
- analyst-review casework surfaces through `alert_queue_policy.json`, `alert_queue_rankings.json`, `analyst_review_scorecard.json`, `case_packet.json`, and `review_capacity_sensitivity.json`, so Relaytic-AML can rank entity cases under bounded review budgets, justify queue order, and hand a human reviewer one evidence-backed case packet instead of only a flat suspicion score
- streaming AML risk surfaces through `stream_risk_posture.json`, `weak_label_posture.json`, `delayed_outcome_alignment.json`, `drift_recalibration_trigger.json`, and `rolling_alert_quality_report.json`, so Relaytic-AML can make weak-label risk, delayed confirmation, rolling alert pressure, and recalibration triggers visible instead of pretending static supervised labels tell the full production story
- AML baseline and capability-ablation surfaces through `relaytic aml baselines`, with baseline matrices, adapter fallback reports, no-graph/no-temporal/no-review-budget/no-calibration/no-typology-prior ablations, capability-contribution summaries, and supported/proxy/blocked benchmark relevance scorecards
- AML graph-loader surfaces through `relaytic aml graph-loader`, with raw Elliptic-style multi-file bundle detection, edge/feature/label/time provenance, incomplete-bundle recovery instructions, flattened graph compatibility labeling, subgraph task manifests, graph claim scope, and a public graph benchmark catalog that separates raw graph, flattened proxy, Elliptic2-style subgraph, and AMLSim-style evidence
- AML proof-pack surfaces through `aml_benchmark_manifest.json`, `aml_holdout_claim_report.json`, `aml_demo_scorecard.json`, `aml_public_claim_guard.json`, and `aml_failure_report.json`, so PaySim-style and flattened Elliptic-style runs can expose dataset family, cross-track coverage, demo readiness, allowed versus blocked claims, and the primary remaining proof gap through benchmark CLI/show, run summary, assist, and mission control
- AML flagship demo surfaces through `relaytic demo aml-review-queue`, `aml_demo_bundle_manifest.json`, `aml_demo_business_metric_table.json`, `aml_demo_flow_report.md`, `aml_demo_artifact_index.json`, and `aml_investigation_board.json`, so a reviewer can run one public-safe demo and see the queue, top case, drift posture, benchmark guard, public-claim guard, and failure posture without reading raw artifacts first
- freshness-aware artifact reuse through `relaytic runtime reuse`, with explicit dependency graphs, freshness contracts, recompute plans, cache indexes, invalidation reports, and stage-level reuse decisions that benchmark review, completion review, and trace replay all share
...
- demo-grade onboarding through explicit guided demo flow, mode explanations, stuck-recovery guidance, and a public-safe walkthrough surfaced directly from mission control, chat, and the handbook stack
...
- explicit release safety via `relaytic scan-git-safety`, with workspace-only pre-release scans, built-bundle attestation, source-map auditing, sensitive-string and machine-path scanning, package inventory capture, packaging-regression reporting, and doctor-visible release posture
- pre-Academy repo credibility reporting under `docs/reports/`, with module-size audits, public-surface inventory, module-split evidence, retained extraction boundaries, and benchmark-surface cleanup debt for the paper-freeze handoff
- paper/release freeze reporting through `relaytic release-safety paper-freeze`, with AML benchmark relevance catalog, multidimensional paper result table, explicit public-claim boundaries, reproducibility attestation, and a safe attention-pack manifest that blocks hard AML performance claims until holdout evidence and claim gates pass
- paper operational metric reporting through `relaytic release-safety paper-operational-metrics`, with supporting PaySim and Elliptic review-budget rows, false-positive burden proxies, case-packet completeness state, and a fail-closed operational claim guard before P10 table generation
- reproducible paper table generation through `relaytic release-safety paper-tables`, with `paper_result_table_final.json`, metric-cell provenance, reproduction commands, audit checks, and a publishability matrix that keeps hard claims blocked until later gates pass
- claim-linted paper drafting through `relaytic release-safety paper-draft`, with `docs/paper/relaytic_aml_draft.md`, four generated figures, a limitations matrix, and a draft claim-lint report that checks sections, metric-cell references, limitation coverage, figure provenance, and blocked-claim wording before P12
- external dry-run proof through `relaytic release-safety paper-dry-run`, with a clean-clone checklist, optional isolated full-profile install probe, P10/P11 paper-smoke regeneration, leak-scan status, reproduction failure reporting, and a P13 go/no-go gate
- claim-safe paper release packaging through `relaytic release-safety paper-release`, with `docs/paper/relaytic_aml_arxiv_draft.md`, `docs/paper/references.bib`, generated paper tables, `paper_release_manifest.json`, `paper_arxiv_submission_checklist.md`, `paper_attention_pack.md`, and `paper_public_claims_allowed.json`
- final arXiv source-bundle packaging through `relaytic release-safety paper-arxiv-source`, with `docs/paper/arxiv_src/main.tex`, `docs/paper/arxiv_src/references.bib`, converted PDF figures, `paper_arxiv_source_manifest.json`, `paper_submission_package_audit.json`, and `paper_release_candidate_checklist.md`
- external score-file governance proof through `relaytic release-safety paper-external-score-proof`, with `paper_external_score_route_decision.json`, `paper_external_score_schema.json`, `paper_external_score_manifest.json`, `paper_external_score_evidence_cells.json`, `paper_external_score_claim_gate.json`, `paper_external_score_handoff_eval.json`, and `paper_external_score_summary.md`
- hosted score case-study integration through `relaytic release-safety paper-external-score-integration`, with `paper_external_score_case_study.json`, `paper_external_score_paper_panel.json`, `paper_external_score_claim_map.json`, `paper_external_score_repro_card.md`, and `paper_external_score_integration_manifest.json`
...
- canonical objective-and-split truth through `optimization_objective_contract.json`, `objective_alignment_report.json`, `split_diagnostics_report.json`, `temporal_fold_health.json`, `metric_materialization_audit.json`, and `benchmark_truth_precheck.json`
- benchmark review that can now fail closed when the comparison metric is not materialized honestly or temporal validation/test folds are degenerate, instead of emitting a misleading ranking
...
- canonical architecture-routing artifacts through `architecture_registry.json`, `architecture_router_report.json`, `candidate_family_matrix.json`, `architecture_fit_report.json`, `family_capability_matrix.json`, and `architecture_ablation_report.json`
- widened trainable architecture support for histogram-gradient boosting and extra-trees regression/classification families, plus optional CatBoost, XGBoost, LightGBM, and TabPFN adapters when installed
- run-summary, planning, inference, assist, and memory surfaces that now share one explicit architecture-routing contract instead of treating family choice as an opaque Builder side effect
- governed imported-architecture evaluation through `method_import_report.json`, `architecture_candidate_registry.json`, `shadow_trial_manifest.json`, `shadow_trial_scorecard.json`, `candidate_quarantine.json`, and `promotion_readiness_report.json`
- real shadow-trial comparison for supported imported families so research-driven candidates can prove themselves on the same local benchmark split before they influence live routing
- assist, benchmark, search, and run-summary surfaces that can explain why an imported family stayed shadow-only, candidate-available, promotion-ready, or quarantined instead of treating research intake as prose-only context
- canonical temporal-engine artifacts through `temporal_structure_report.json`, `temporal_feature_ladder.json`, `rolling_cv_plan.json`, `temporal_split_guard_report.json`, `sequence_shadow_scorecard.json`, `temporal_baseline_ladder.json`, and `temporal_metric_contract.json`
- time-aware runs that now preserve future binary events when possible, materialize richer lag/delta/rolling/seasonal temporal feature families, compare honest ordinary-versus-lagged baselines, materialize temporal metric aliases cleanly in benchmark truth, and keep sequence candidates shadow-only until they beat a strong lagged baseline under the same contract
...
- the completed Paper Track P0 through P24, including the P24 protocol, bibliography, metric/split/semantic, visual, PDF/source, and exact-revision release-integrity closure
