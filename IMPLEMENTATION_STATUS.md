...
- a mandatory pre-Academy productization and paper-freeze track now remaining through Slices 15Z through 15Z-R for repo credibility cleanup and a relevant benchmark/release freeze
...
With Slice 15Y now landed and the AML demo-first documentation path explicit, the next high-leverage frontier follow-ons are:
...
- Slice 16A capability registry and capability cards so future academy work starts from one governed source of truth instead of ad hoc candidate state, but only after Slices 15Z and 15Z-R are complete
