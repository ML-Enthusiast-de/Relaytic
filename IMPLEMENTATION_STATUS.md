...
- latest paper-track slice: P6, `paper_baseline_suite_manifest.json`, `paper_tabular_baseline_table.json`, `paper_benchmark_budget_contract.json`, `paper_leakage_safe_feature_report.json`, `paper_publishability_gate.json`, and Paper Track P6-A as the next target
- latest paper-PaySim slice: P6, full 6,362,620-row chronological baseline suite with six executed families, train-only leakage-safe features, explicit adapter versions/fallbacks, Extra Trees selected on validation (`test_pr_auc=0.331345`), and headline/hard claims still blocked until P6-A
...
- next planned academy follow-on: 16A, capability registry and capability cards, after Paper Track P6-A through P13 complete competitive-budget, graph benchmark, paper draft, dry run, and arXiv proof path
...
With Slice 15Z-R and Paper Track P0-P6 now landed, the next high-leverage frontier follow-ons are:

- Paper Track P6-A, challenge the clean PaySim baseline suite with a declared competitive feature/HPO/calibration budget and a publishability gate
- Paper Track P7 through P13, add graph baselines, AMLSim/Elliptic2 support decisions, operational metrics, reproducible tables, a claim-linted paper draft, clean-clone dry run, and only then an arXiv release
