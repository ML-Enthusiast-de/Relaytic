...
- a mandatory pre-Academy productization and paper-freeze track through Slices 15V-A through 15Z-R for no-lost human/agent guidance, safe external context export, temporal weak-label upgrades, evaluation-environment framing, demo-first docs, repo credibility cleanup, and a relevant benchmark/release freeze
...
With Slice 15V now landed and AML graph-loader proof explicit, the next high-leverage frontier follow-ons are:

- Slice 15V-A no-lost guide and external context pack so inexperienced humans and external agents can always ask where Relaytic is, what is possible now, what is blocked, what to do next, and what safe context to hand to another LLM
- Slice 15W temporal and weak-label upgrade so delayed labels, positive-unlabeled posture, threshold drift, and rolling windows become stronger proof surfaces
...
- Slice 16A capability registry and capability cards so future academy work starts from one governed source of truth instead of ad hoc candidate state, but only after Slices 15V-A through 15Z-R are complete
