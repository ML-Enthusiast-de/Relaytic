...
class _EvidenceLLMHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        content = {
            "belief_shift": "The baseline is viable, but the challenger pressure means Relaytic should keep a human in the loop before promotion.",
            "audit_focus": ["Challenger pressure", "Operating-envelope caution"],
            "next_actions": ["Review the challenger branch before promotion."],
            "memo_note": "Do not present the baseline as final without acknowledging the challenger pressure.",
        }
...
class _LocalServer:
    def __init__(self) -> None:
        _EvidenceLLMHandler.last_request = None
        self._server = ThreadingHTTPServer(("127.0.0.1", 0), _EvidenceLLMHandler)
...
def test_run_evidence_review_uses_local_llm_advisory_when_enabled(tmp_path: Path) -> None:
    data_path = _write_dataset(tmp_path / "slice06_llm.csv")
    run_dir = tmp_path / "run_slice06_llm"
    ...
    assert result.bundle.audit_report.llm_advisory is not None
    assert "human in the loop" in result.bundle.belief_update.updated_belief.lower()
    assert _EvidenceLLMHandler.last_request is not None
