...
class _PlanningLLMHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        content = {
            "alternative_routes": [
                {
                    "route_id": "advisory_threshold_sweep",
                    "title": "Threshold stress-test branch",
                    "tradeoff": "higher_calibration_review_cost",
                    "reason": "Decision usefulness may change materially if the threshold is tuned for a stricter early-warning posture.",
                }
            ],
            "extra_hypotheses": [
                "Small drift-normalized sensor deltas may improve early warning reliability.",
            ],
            "experiment_clues": [
                "Run a threshold sweep against the same route before expanding model complexity.",
            ],
            "marginal_value_hint": "Threshold tuning is likely to change operator-facing recall/precision tradeoffs more than a new family swap.",
        }
...
class _LocalServer:
    def __init__(self) -> None:
        _PlanningLLMHandler.last_request = None
        self._server = ThreadingHTTPServer(("127.0.0.1", 0), _PlanningLLMHandler)
...
def test_run_planning_uses_local_llm_advisory_when_enabled(tmp_path: Path) -> None:
    data_path = _write_dataset(tmp_path / "planning_llm.csv")
    ...
    assert planning_bundle.plan.llm_advisory is not None
    assert planning_bundle.plan.trace.llm_used is True
    assert any(item["alternative_id"] == "advisory_threshold_sweep" for item in planning_bundle.alternatives.alternatives)
    assert any(item["category"] == "advisory" for item in planning_bundle.hypotheses.hypotheses)
    assert any(item["experiment_id"] == "advisory_experiment_01" for item in planning_bundle.experiment_priority_report.prioritized_experiments)
    assert "Advisory note" in planning_bundle.marginal_value_of_next_experiment.rationale
    assert _PlanningLLMHandler.last_request is not None
